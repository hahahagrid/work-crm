...
## 🚀 Current Infrastructure State (Текущее состояние проекта)
*Обновляйте эту секцию при значительных изменениях инфраструктуры.*
- **Репозиторий:** Привязан к удаленному `hahahagrid/work-crm`.
- **Git ветки:** Основная разработка идет в `dev`. Ветки синхронизированы.
- **Окружение:** `node_modules` скрыты `.gitignore`, шаблон `.env.example` создан, зависимости `npm install` установлены.
- **Качество кода:** Настроены глобальный Prettier и локальный ESLint (для `crm-app`).
- **Бэкенд:** Выбрана БД PostgreSQL (Railway). Ожидается предоставление подключения.
